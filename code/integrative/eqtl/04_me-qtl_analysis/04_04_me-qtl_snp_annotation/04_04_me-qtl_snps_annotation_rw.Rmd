...
out.dir.pre.pre  <- "~/bio/code/mpip/dex-stim-human-array/output/data/integrative/matrixEQTL/"
...
delta.meqtls.snp.chromhmm.annotated.df <- fread("~/bio/code/mpip/dex-stim-human-array/output/data/integrative/matrixEQTL/region_wise_independent_snps/meqtls_snp_chromhmm_annotated_delta.csv")
...
chromhmm.blood.enrich.perm.rslt <- read.csv2(paste0(out.dir.pre, "meqtl_snps_chromHMM_blood_enrichment_perm_20.csv"))
chromhmm.brain.enrich.perm.rslt <- read.csv2(paste0(out.dir.pre, "meqtl_snps_chromHMM_brain_enrichment_perm_100.csv"))
...
chromhmm.blood.enrich.perm.rslt["prop"] <-  chromhmm.blood.enrich.perm.rslt$n_snps_overlap / delta.meqtl.snp.anno.rds@peakNum

ggplot(chromhmm.blood.enrich.perm.rslt, aes(x = state, y = prop)) + 
  geom_bar( stat = "identity", position = position_dodge()) +
  geom_text(aes(label = n_snps_overlap), position=position_dodge(width=0.9), vjust=-0.25) + 
  labs(x = "",
       y = "Overlaps", 
       title = "") +
  theme(legend.position = "bottom",
...
chromhmm.brain.enrich.perm.rslt["prop"] <-  chromhmm.brain.enrich.perm.rslt$n_snps_overlap / delta.meqtl.snp.anno.rds@peakNum
...
```{r eval = T, include = F}
out.dir.pre  <- "~/bio/code/mpip/dex-stim-human-array/output/data/integrative/matrixEQTL/05_me-qtl_enrichment/region_wise_independent_snps/"
gwas.enrich.perm.rslt <- read.csv2(paste0(out.dir.pre, "meqtl_snps_GWAS_CD_2019_enrichment_perm_100.csv"))
...
```{r out.width = "90%", fig.cap = "Enrichment of the the delta meSNPs for CrossDisorder GWAS. The Y-axis denotes the proportion of overlaps between meSNPs and GWAS SNPs.", eval = T, include = T}

gwas.enrich.perm.rslt["prop"] <-  gwas.enrich.perm.rslt$n_snps_overlap / delta.meqtl.snp.anno.rds@peakNum
