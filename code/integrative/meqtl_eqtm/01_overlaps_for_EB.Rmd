...
meth.sub.df   <- meth.beta.dex.mtrx[CpG_ID %in% venn.rslt$cpgs]

meqtm.gex.ids <- eqtm.dex.nom.df[CpG_ID %in% venn.rslt$cpgs, ENSG_ID] %>% unique()
gex.sub.df    <- gex.dex.mtrx[ENSG_ID %in% meqtm.gex.ids]

meth.cor.mtrx <- cor(t(meth.sub.df[, -1]))
gex.cor.mtrx  <- cor(t(gex.sub.df[, -1]))

meth.gex.sub.df   <- rbind(meth.sub.df[1:n.subset, -1], gex.sub.df[1:n.subset, -1])
...
corrplot(meth.cor.mtrx[1:n.subset, 1:n.subset], 
         method = "color", cl.pos = "r", 
         type = "full",
         tl.pos = "n")
```
...
corrplot(gex.cor.mtrx[1:100, 1:100], 
         method = "color", cl.pos = "r", 
         type = "full",
         tl.pos = "n")
```
...
eqtm.id       <- eqtm.dex.nom.df[fdr_eqtm == min(fdr_eqtm), eQTM_ID]
selected.eqtm <- eqtm.dex.nom.df[eQTM_ID %in% eqtm.id] # beta == max(beta)

meqtl.gr <- fread("~/bio/code/mpip/dex-stim-human-array/output/data/integrative/matrixEQTL/meqtls/dex_meqtl_with_veh_primary_group_with_delta.csv")

meqtl.gr <- fread("~/bio/code/mpip/dex-stim-human-array/output/data/integrative/matrixEQTL/meqtls/dex_veh_meqtl_opposite_beta_9K.csv")

meqtl.gr <- fread("~/bio/code/mpip/dex-stim-human-array/output/data/integrative/matrixEQTL/meqtls/dex_veh_meqtl_opposite_beta_group_with_delta.csv")

selected.meqtl <- meqtl.gr[CpG_ID %in% selected.eqtm$CpG_ID]
...
ProcessGetBoxPlot(meth.beta.veh.mtrx, meth.beta.dex.mtrx, snp.mtrx, selected.eqtm, fdr.thr = 0.05, 
