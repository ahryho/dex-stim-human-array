...
ggplot(meqtl.cpg.anno.df[Model %in% c("Delta")], aes(x = Model, fill = Relation_to_Island)) + 
  ...
  geom_text(aes(by = Model, # scales::percent(..count../tapply(..count.., ..x.., sum), accuracy = 0.1), 
                ...
                stat = "prop", 
                position = position_stack(vjust = 0.9, ), size = 3.5, color = "white") +x
...
ggplot(meqtl.snp.anno.stat.df[meqtl.snp.anno.stat.df$Model == "Delta", ], aes(x = Model, y = Frequency, fill = Feature)) + 
  ...
  geom_text(
    ...
    aes(by = Model, label = paste0(Frequency, "%")), position = position_stack(vjust = 0.9, ), size = 3.5, color = "white") +
  coord_polar(theta = 'y', start = 0) +
  labs(x = " ",
       y = "Percentage of CpGs", 
       title = "") +
...
```{r}
gen.loc.enrich.perm.rslt <- fread("/binder/mgp/workspace/2020_DexStim_Array_Human/dex-stim-human-array/output/data/integrative/matrixEQTL/05_me-qtl_enrichment/region_wise_independent_snps/meqtl_cpgs_chipseeker_enrichment_perm_delta_vs_veh.csv")
```
