...
# Calculate homo major / minor allele
...
colnames(ind.meqtl.delta.df.2)[8] <- "methyl_change"
...
intersect.meqtl <- data.frame(meQTL_ID = intersect(methyl.change.dex.df$meQTL_ID, methyl.change.veh.df$meQTL_ID))
...
methyl.change.plt.df %>%
  ggplot(aes(y = Mean, x = Genotype, fill = treatment)) +
    ...
    labs(y = "Beta methylation, mean", title = "Intersect meQTLs")
```