...
```{r}
sign.pval.df <- pval.veh.bcc.df %>% reshape2::melt(measure.vars = colnames(pval.veh.df)[2:13]) %>% setDT()
colnames(sign.pval.df) <- c("CpG_ID", "Type", "pval")
sign.pval.df <- sign.pval.df[pval <= 0.05]
...
```{r}
sign.pval.df <- pval.dex.bcc.df %>% reshape2::melt(measure.vars = colnames(pval.dex.df)[2:13]) %>% setDT()
colnames(sign.pval.df) <- c("CpG_ID", "Type", "pval")
sign.pval.df <- sign.pval.df[pval <= 0.05]
