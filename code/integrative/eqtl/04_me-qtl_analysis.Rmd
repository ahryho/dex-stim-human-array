...
The meQTL analysis led to 
  + __123'399'492__ dex cis-meQTLs
  + __123'573'064__ veh cis-meQTLs
...
hist(plyr::count(meqtl.dex.full.df$SNP)$freq, 
     breaks = 15,
     main = paste0("Distribution of CpGs per SNPs (dex) fdr <= ", fdr.thr),
     xlab = 'Nr of CpGs')
```
...
hist(plyr::count(meqtl.veh.full.df$SNP)$freq, 
     breaks = 10,
     main = paste0("Distribution of CpGs per SNPs (veh) fdr <= ", fdr.thr),
     xlab = 'Nr of CpGs')
```
...
```{r out.width = "95%", include = T, eval = T}
  ...
  ggplot(meqtl.all.full.df, aes(x = beta, y = `p-value`, shape = treatment, color = treatment)) +
   # geom_violin(width = 0.4) +
    geom_point() +
