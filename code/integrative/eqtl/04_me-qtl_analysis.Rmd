...
The correction for treatment, sex, case / control status, age, BMI,  smoking score, first two principal components (PCs) for population stratification and first three significant Surrogate Variables (SVs) was done. 

For cis-QTL analyses, false discovery rate (FDR) of 5% was chosen. 

```{sh}
meqtl_rslt_dir=
awk 'NR == 1 || $6 < 0.05' $meqtl_rslt_dir/me-qtl_cis_result_dex.csv > $meqtl_rslt_dir/me-qtl_cis_result_dex_fdr_005.csv
```
...
hits.df %>% as.matrix() %>% reshape2::melt(var.names = c("ID", "Treatment"), value.name = "cnt") %>%
  ggplot(aes(y = cnt, x = Var2, fill = as.factor(as.numeric(Var2)))) +
   # geom_violin(width = 0.4) +
    geom_col() +
    geom_text(aes(label = cnt, y = cnt), stat = "identity", 
          vjust = -0.5, size = 3) +
...
ggplot(plt.df, aes(x = Frequency, fill = treatment)) + 
  geom_histogram(aes(color = treatment),  bins = 15, alpha = 0.5) +
  theme(legend.position = "none", 
        legend.title = element_blank(),
        panel.grid.major = element_blank(),
        panel.background = element_blank(),
...
```{r out.width = "95%", include = T, eval = T}
hist(plyr::count(meqtl.dex.full.df$SNP)$freq, 
     breaks = 15,
     main = paste0("Distribution of CpGs per SNPs (delta) fdr <= ", fdr.thr),
     xlab = 'Nr of CpGs')
```
...
```{r out.width = "95%", include = T, eval = T}
hist(plyr::count(meqtl.veh.full.df$SNP)$freq, 
     breaks = 10,
     main = paste0("Distribution of CpGs per SNPs (delta) fdr <= ", fdr.thr),
     xlab = 'Nr of CpGs')
```
...
ggplot(bins.df, aes(DistanceInterval, fill = treatment)) + 
  ...
  stat_count(geom = "text", 
             ...
             position = position_dodge(1),  vjust = -1, colour = "black", cex = 3) +
