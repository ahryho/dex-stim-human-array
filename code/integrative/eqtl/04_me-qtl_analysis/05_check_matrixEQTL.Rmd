...
cpg.vec <- t(beta.mtrx.delta[cg, ])

# Create 3 SlicedData objects for the analysis
snps1 <- SlicedData$new( matrix( pheno.dex$SNP, nrow = 1 ) );
...
dex.lm <- lm(CpG_ID ~ SNP + Sex + Age + BMI_D1 + Status + DNAm_SmokingScore + DNAm_SV1 + DNAm_SV2 + DNAm_SV3 + PC1 + PC2, data = pheno.dex)
