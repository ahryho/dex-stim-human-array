...
meqtl.delta.fn <- paste0(out.dir.pre, "eqtm_cis_result_delta_beta.csv") 
...
veh.full.df   <- GetFulleQTMdf(veh.df, fdr.thr = fdr.thr, treatment = "veh")
...
all.full.df   <- rbind(dex.full.df, veh.full.df, delta.full.df)
```
...
```{r out.width = "95%", include = T, eval = T}
plot.title <- paste0("Independent and significant at FDR <= ", fdr.thr, " QTMs.",
...
eqtms.ensg <- list(delta = delta.full.df$ENSG_ID, dex = dex.full.df$ENSG_ID, veh = veh.full.df$ENSG_ID)

upset(fromList(eqtms.ensg), 
      sets = c("delta", "dex", "veh"),
      nsets = 3, 
      nintersects = 20, 
      mainbar.y.label = "Number of intersections, ENSG", 
      text.scale = 1, 
      keep.order = T,
      sets.bar.color = c("#0072B2", "#009E73", "#E69F00"), 
      order.by = "freq")
...
eqtms.cpg <- list(delta = delta.full.df$CpG_ID, dex = dex.full.df$CpG_ID, veh = veh.full.df$CpG_ID)
...
## __Significant results, FDR <= 0.05__
