...
methyl.change.dex.df <- left_join(methyl.change.df, ind.meqtl.dex.df)
methyl.change.veh.df <- left_join(methyl.change.df, ind.meqtl.veh.df)
...
ggplot(methyl.change.df.plt, aes(x = methyl_change, y = -log10(fdr), shape = treatment, color = treatment)) +
  geom_point(alpha = 1.5, size = 1.2) +
  scale_x_continuous(labels = scientific) +
  geom_vline(xintercept = 0, colour = "#990000", linetype = "dashed") + 
  geom_hline(yintercept = -log10(fdr.thr), colour = "#990000", linetype = "dashed") +
...
# methyl.change.df.plt.tmp <- methyl.change.df[treatment == "dex" & CpG_ID %in% mad.cpgs.dex]

methyl.change.df.plt.2 <- data.frame(rbind(
...
methyl.change.df.plt.2 <- left_join(methyl.change.df.plt.2, freq, by = c("SNP" = "ID")) %>% setDT()
methyl.change.df.plt.2 <- methyl.change.df.plt.2[ALT_FREQS >= maf.thr ]

ggplot(methyl.change.df.plt.2, aes(x = methyl_change, y = -log10(fdr), shape = treatment, color = treatment)) +
  geom_point(alpha = 1.5, size = 1.2) +
  scale_x_continuous(labels = scientific) +
  geom_vline(xintercept = 0, colour = "#990000", linetype = "dashed") + 
  geom_hline(yintercept = -log10(fdr.thr), colour = "#990000", linetype = "dashed") +
