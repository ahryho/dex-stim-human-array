...
```{r include = F, eval = T}
cbPalette <- c(delta = "#0072B2", dex = "#009E73", baseline = "#E69F00")
```
...
```{r include = T, eval = T}
meqtl.id       <-  meqtl.all.full.df[treatment == "delta"][order(fdr, decreasing = T)][3, meQTL_ID]
...
```{r, eval = T, include = F}
plot.title <- paste0(# "Independent meQTL ", selected.meqtl$meQTL_ID, 
                     "meQTL with maximum FC across GR-meQTLs: ",
                     "FC = ", signif(selected.meqtl$beta, 2), " and FDR = ", signif(selected.meqtl$fdr, 2))
```
```{r out.width = "95%", include = T, eval = T}
GetScatterPlot(meqtl.all.full.df, selected.meqtl, fdr.thr = 0.05,
...
```{r}
ggplot(allel.dir.fc.delta.meqtls, aes( x = 1, fill = Group, colour = Group)) + 
  geom_bar(position = "fill", stat = "count", alpha = 0.5) + 
  coord_flip() +
  stat_count(geom = "text",
             aes(label = scales::percent((..count..)/sum(..count..), accuracy = 0.1)),
             angle = 0,  vjust = 0.5, hjust = 1.1, colour = "black", cex = 3) +
  stat_count(geom = "text",
             aes(label = ..count..),
             angle = 0,  vjust = -1, hjust = 1.5, colour = "black", cex = 3) +
  theme(legend.position = "bottom", 
        legend.title = element_blank(),
        panel.grid.major = element_blank(),
        panel.background = element_blank(),
...
selected.meqtl <- meqtl.all.full.df[meQTL_ID %in% meqtl.id][fdr == min(fdr)]
```

```{r, eval = T, include = F}
plot.title <- paste0(# "Independent meQTL ", selected.meqtl$meQTL_ID, 
                     "The most significant meQTL with opposite FC: ",
                     "delta FC = ", signif(selected.meqtl[treatment == "delta"]$beta, 2), " and delta FDR = ", signif(selected.meqtl[treatment == "delta"]$fdr, 2))
```
...
```{r, eval = T, include = F}
plot.title <- paste0(# "Independent meQTL ", selected.meqtl$meQTL_ID, 
                     "The most significant meQTL with parallel FC: ",
                     "delta FC = ", signif(selected.meqtl[treatment == "delta"]$beta, 2), " and delta FDR = ", signif(selected.meqtl[treatment == "delta"]$fdr, 2))
```
...
bins.df <- data.frame(DistanceInterval = bin_data(data.table::data.table(meqtl.delta.full.df), binCol = "dist", bins = c(-1e6 - 1, -1e-1, -1e4 0, 1, 1e3, 1e4+1, 1e5+1, 1e6+1), returnDT = F))
levels(bins.df$DistanceInterval) <- c("0", "(200; 1e3]", "(1e3; 1e4]", "(1e4; 1e5]", "(1e5; 1e6]")
...
ggplot(bins.df, aes(DistanceInterval)) + 
  ...
  labs(title = paste0("Distribution of cis-meQTLs, FDR = ", fdr.thr), 
       ...
       y = "No. cis-meQTLs") + 
  scale_fill_manual(values = cbPalette[["delta"]])
```
...
```{r out.width = "100%", include = T, eval = T}
ggplot(meqtl.delta.full.df, aes(x = dist)) + 
  geom_density(aes(y = ..count..), colour = cbPalette[["delta"]]) +
  theme(legend.position = c(.1,.9), 
        legend.title = element_blank(),
        panel.grid.major = element_blank(),
        panel.background = element_blank(),
...
ggplot(meqtl.delta.full.df, aes(x = dist, y = -log10(fdr))) + 
  geom_point(colour = cbPalette[["delta"]]) +
