...
## Delta with maximum positive FC

```{r include = T, eval = T}
# summary(meqtl.all.full.df[treatment == "dex"])
selected.meqtl <- meqtl.all.full.df[treatment == "delta"][beta == max(beta)] # beta == max(beta)
kable(selected.meqtl %>% 
        select(SNP, CpG_ID, FC = beta, `t-stat`, `p-value`, FDR = fdr, Treatment = treatment) %>% 
        mutate_if(is.numeric, funs(as.character(signif(., 3)))))
```
...
## Delta with maximum negative FC

```{r include = T, eval = T}
# summary(meqtl.all.full.df[treatment == "dex"])
selected.meqtl <- meqtl.all.full.df[treatment == "delta"][beta == min(beta)] # beta == max(beta)
kable(selected.meqtl %>% 
        select(SNP, CpG_ID, FC = beta, `t-stat`, `p-value`, FDR = fdr, Treatment = treatment) %>% 
        mutate_if(is.numeric, funs(as.character(signif(., 3)))))
```
...
## Delta with minimum absolute FC

```{r include = T, eval = T}
# summary(meqtl.all.full.df[treatment == "dex"])
selected.meqtl <- meqtl.all.full.df[treatment == "delta"][abs(beta) == min(abs(beta))] # beta == max(beta)
kable(selected.meqtl %>% 
        select(SNP, CpG_ID, FC = beta, `t-stat`, `p-value`, FDR = fdr, Treatment = treatment) %>% 
        mutate_if(is.numeric, funs(as.character(signif(., 3)))))
```
...
## Only delta, but not VEH or DEX, min FDR
...
selected.meqtl <- meqtl.all.full.df[meQTL_ID %in% only.delta.meqtls][fdr == min(fdr)] # beta == max(beta)
kable(selected.meqtl %>% 
        select(SNP, CpG_ID, FC = beta, `t-stat`, `p-value`, FDR = fdr, Treatment = treatment) %>% 
        mutate_if(is.numeric, funs(as.character(signif(., 3)))))
```
...
## Only DEX, but not DELTA or VEH, min FDR
...
only.dex.meqtls <- setdiff(meqtl.all.full.df[treatment == "dex", meQTL_ID], 
                             union(meqtl.all.full.df[treatment == "delta", meQTL_ID],
                                   meqtl.all.full.df[treatment == "veh", meQTL_ID]))
selected.meqtl <- meqtl.all.full.df[meQTL_ID %in% only.dex.meqtls][fdr == min(fdr)] # beta == max(beta)
kable(selected.meqtl %>% 
        select(SNP, CpG_ID, FC = beta, `t-stat`, `p-value`, FDR = fdr, Treatment = treatment) %>% 
        mutate_if(is.numeric, funs(as.character(signif(., 3)))))
```
...
## Only VEH, but not DEX or DELTA, min FDR
