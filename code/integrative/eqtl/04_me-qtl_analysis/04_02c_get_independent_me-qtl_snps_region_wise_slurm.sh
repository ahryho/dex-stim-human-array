...
#SBATCH --mem-per-cpu=200M     # Each task uses max 2G of memory
#SBATCH --array=1-7500%100   # Submit 136974 (137537) tasks (number of dex meCpGs). Run max 100 concurrently
#SBATCH --part=pe
...
rslt_dir=/home/ahryhorzhevska/mpip/bio/code/mpip/dex-stim-human-array/output/data/integrative/matrixEQTL/region_wise_indepe$
treatment=veh

# test.txt
# awk 'NR != 1 {print $2}' me-qtl_cis_result_${treatment}_fdr_005.csv | sort | uniq > list_${treatment}_cpgs_for_clumping.c$
...
awk -v cg=$cpg '$2 ~ cg {print $1 " " $5}' ${rslt_dir}/me-qtl_cis_result_${treatment}_fdr_005.csv >> ${treatment}_snp_lst_f$

plink --bfile $snp_dir/Dex_genoData_SNPs --clump ${treatment}_snp_lst_for_ld_clump_${cpg}.txt --clump-r2 0.2 --clump-kb 200$

# echo "CpG_ID SNP" >  me-qtl_cis_${treatment}_ind_cpg_snp_associations_${cpg}.txt
awk -v cg=$cpg 'NR != 1 {if($3) print cg " " $3}' ${treatment}_${cpg}.clumped >> ${rslt_treatment_dir}/me-qtl_cis_${treatme$
...
# cp ${SCRATCH_DIRECTORY}/me-qtl_cis_${treatment}_ind_cpg_snp_associations.txt ${rslt_treatment_dir}
