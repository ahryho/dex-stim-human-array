...
selected.meqtl <- meqtl.parallel.fc.grp.delta.df[CpG_ID %in% selected.eqtm$CpG_ID, 
                                                 .(meQTL_ID, CpG_ID, SNP, beta, `p-value`, fdr, treatment)] %>% unique()

selected.eqtl <- data.frame(CpG_ID = selected.eqtm$ENSG_ID, SNP = selected.meqtl$SNP) %>% unique()
...
```{r, eval = T, include = F}
plot.title <- "Parallel FC dex"
```

```{r out.width = "95%", include = T, eval = T}
plt.df <- data.frame(CpG = c(as.numeric(meth.beta.dex.mtrx[CpG_ID == selected.eqtm$CpG_ID, -1]),
                             as.numeric(meth.beta.veh.mtrx[CpG_ID == selected.eqtm$CpG_ID, -1])), 
                     ENSG = as.numeric(gex.dex.mtrx[ENSG_ID == selected.eqtm$ENSG_ID, -1],
                                       gex.veh.mtrx[ENSG_ID == selected.eqtm$ENSG_ID, -1]), 
...
plt.df %>%
  ...
  xlab(selected.eqtm$CpG_ID) + ylab(selected.eqtm$ENSG_ID) +
  theme( 
